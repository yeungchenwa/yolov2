...
class YOLOv2:
    ...
    def _define_inputs(self):
        ...
        mean = tf.convert_to_tensor([123.68, 116.779, 103.979], dtype=tf.float32)       #VOC2007像素均值
        ...
        if self.mode == 'train':
            ...
            self.images = self.images - mean    #normilazation
        ...
        for i in range(self.num_gpu):
            with tf.device('/gpu:%d' % i):
                ...
                with tf.variable_scope(('head'), reuse=tf.compat.v1.AUTO_REUSE):    # get yolov2 head
                    ...
                    pclass = pred[..., :self.num_classes]                           # 每个class的概率
                ...
                for i in range(self.batch_size):
                    ...
                    rgaindex = tf.expand_dims(tf.cast(tf.argmax(rgaiou, axis=-1), tf.int32), -1)    # 取最大值iou的anchor下标
                    rgaindex = tf.concat([tf.expand_dims(tf.range(tf.shape(rgaindex)[0]), -1), rgaindex], axis=-1)     #修正下标表达[0,anchor_index]
                    ...
                    hw_loss = 0.5 * tf.reduce_sum(tf.square(tf.exp(rpbbox_hw) - rpbbox_hw_target))
                    coord_loss = yx_loss + hw_loss
                    #class_loss obj_loss
                    class_loss = tf.reduce_sum(tf.square(gn_labeli - tf.nn.sigmoid(rpclass)))
    ...
    def _create_saver(self, path_backone, path_head):
        ...
        self.pretraining_weight_saver1 = tf.compat.v1.train.Saver(weights_backone, max_to_keep=2)
        self.pretraining_weight_saver2 = tf.compat.v1.train.Saver(weights_head, max_to_keep=2)
    ...
    def train_one_epoch(self, lr, writer=None):
        ...
        self.sess.run(self.train_initializer)
        mean_loss = []
        num_iters = self.num_train // (self.batch_size*2)
    ...
    def test_input(self):
        self.is_training = True
        self.sess.run(self.train_initializer)
        handle = self.sess.partial_run_setup([self.images, self.ground_truth])
        images, ground_truth = self.sess.partial_run(handle, [self.images, self.ground_truth])
